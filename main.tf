...
resource "confluent_environment" "test_flink_env" {
  display_name = "test_flink_env"
...
resource "confluent_kafka_cluster" "my_kafka_cluster" {
  ...
  environment {
    id = confluent_environment.test_flink_env.id
  }

  depends_on = [
    confluent_environment.test_flink_env
  ]
}
...
resource "confluent_flink_statement" "my_flink_statement" {
  ...
  properties = {
    "sql.current-catalog"  = confluent_environment.test_flink_env.display_name
